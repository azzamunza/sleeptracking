const window = { location: { origin: '', pathname: '', search: '' }, supabase: { createClient: () => ({ auth: { onAuthStateChange: () => {}, signInWithOAuth: () => {}, signOut: () => {} }, rpc: () => ({}), from: () => ({ select: () => ({ eq: () => ({ order: () => ({}), maybeSingle: () => ({}) }) }) }) }) }, URL: { createObjectURL: () => {} }, print: () => {} }; const document = { getElementById: () => ({ style: {}, classList: { contains: () => false }, querySelector: () => null }), querySelectorAll: () => [], createElement: () => ({ style: {}, classList: { add: () => {}, remove: () => {} } }) }; const navigator = { serviceWorker: { register: () => Promise.resolve() } }; const alert = () => {}; const Blob = function() {}; 
